...
fn build_buffy(b: *std.Build, target: std.zig.CrossTarget, optimize: std.builtin.Mode) void {
    const raylib = raylib_build.addRaylib(b, target, optimize, .{});
    raylib.defineCMacro("GRAPHICS_API_OPENGL_43", "1");

    const exe = b.addExecutable(.{
        .name = "buffy",
        .root_source_file = .{ .path = "src/buffy.zig" },
        .target = target,
        .optimize = optimize,
    });
    exe.linkLibrary(raylib);
    ...
    const run_step = b.step("buffy", "Run the app");
    run_step.dependOn(&run_cmd.step);
}
